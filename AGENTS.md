...
src/SystemHotkeys/
  ...
  Hotkeys/                  the keyboard hook itself
    ...
    BoundHotkeyCombos.cs      turns GetBindings() into the set of combos currently opted into
                               suppression
    HotkeyPressedPayloads.cs  builds the payload(s) a single press publishes
...
tests/SystemHotkeys.Tests/
  PluginIntegrationTests.cs   the event's shape through the harness; LocalizationTests guards the
                               catalog wiring
  Hotkeys/                    unit tests for the combo/key-name/binding logic above - the actual OS
                               hook is not something a test harness can drive
```
...
- **`BoundHotkeyCombos` is recomputed on two different signals, not one.** `Events.BindingsChanged`
  covers a trigger's combo or suppress toggle being added, edited or removed. A client's position
  (which folder/profile it currently has open) never raises that event, so `PluginIntegration` also
  watches `Deck.ClientChanged` - otherwise a Device/Profile/Folder-scoped suppression would go stale the
  moment someone changes folder.
- **A press fans out to one occurrence per connected client only when scoping could actually change
  which trigger reacts.** `_anyBindingIsScoped` (from `BoundHotkeyCombos.AnyBindingIsScoped`) gates this:
  with nothing scoped, one publish already reaches every trigger, so fanning out would only fire each of
  them once per client for no reason. With at least one scoped binding, `HotkeyPressedPayloads.ForCombo`
  emits one payload per connected client, carrying that client's own device/profile/folder, so the host's
  trigger matching can tell apart a shared combo bound once per folder.
- **`DevicesOptionsSourceId = "macrodeck.devices"` is host-owned, not part of the public SDK.** It is the
  same id the built-in Client Connected/Disconnected triggers use for their own device picker, backed by
  Macro Deck's device registry - the one place a device has the name its owner actually gave it, rather
  than an id nobody recognises. Undocumented and unverified against a real host from here, so it could
  stop resolving without notice on a future host; check the picker still offers real device names after
  any host update.
...
2. **No blocked package is used.** Check every entry in `blockedPackages` against the full dependency
   ...
   way.
3. **The Macro Deck SDK is at least `minimumSdkVersion`.** `MacroDeckSdkVersion` in
   `Directory.Packages.props` is pinned rather than floated (see the comment there for why), so this does
   not hold automatically - check the pinned version against the policy before bumping it, and never
   build a release against an SDK older than the minimum or against `local-feed/`.
...
6. **The plugin passes the conformance suite on every platform it declares.** The Store refuses a build
   ...
   another plugin id or version. The `publish` job in [release.yml](.github/workflows/release.yml) runs
   the suite on a stub host per platform and sends the reports, so never turn that off. Before calling a
   change done, run `macrodeck-plugin test` (see "Verifying a change") and fix every Required failure; a
   failed Recommended check is allowed but worth fixing.
...
The Macro Deck packages are pinned to an exact version in `Directory.Packages.props` rather than
floated, so the commands above need no version argument - see the comment on `MacroDeckSdkVersion` there
for why. Only to test against SDK surface that is not published yet, pack it into `local-feed/` and
